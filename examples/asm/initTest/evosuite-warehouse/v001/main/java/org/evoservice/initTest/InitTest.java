...
/////////////////////////////////////////////////
	...
	void r_timeRule() {
		...
		if (Boolean.TRUE.equals((seconds.get().value < 5))) {
			...
			Time Time2114537280_s = new Time();
			Time2114537280_s.value = (//seconds.get().value
			(seconds.get().value + 1));
			seconds.set(Time2114537280_s);
		} else {
			branch_r_timeRule_2 = true;
			//{ //par
			Time Time190628410_s = new Time();
			Time190628410_s.value = (//seconds.get().value
			0);
			seconds.set(Time190628410_s);
			Time Time1556461159_s = new Time();
			Time1556461159_s.value = (//minutes.get().value
			(minutes.get().value + 1));
			minutes.set(Time1556461159_s);
			//} //endpar
		}
	}
	...
	void r_restartRule() {
		...
		if (Boolean.TRUE.equals((minutes.get().value < 6))) {
			...
			//{ //par
			Time Time1057569470_s = new Time();
			Time1057569470_s.value = (//minutes.get().value
			0);
			minutes.set(Time1057569470_s);
